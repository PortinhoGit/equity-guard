...
def _render_briefing(T: dict) -> None:
    ...
    with st.expander(T["briefing_title"].format(date=today), expanded=True):
        ...
        wa_lines = [
            f"*Briefing Equity Guard*",
            f"*{today}*",
            "",
            f"*Juros*",
            f"EUA Fed: {FED_FUNDS_RATE:.2f}% (FOMC {_fmt_date_br(FED_NEXT_MEETING)})",
            f"BR Selic: {SELIC_RATE:.2f}% (COPOM {_fmt_date_br(SELIC_NEXT_MEETING)})",
            "",
            f"*Commodities*",
