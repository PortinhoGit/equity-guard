...
def _render_briefing(T: dict) -> None:
    ...
    with st.expander(T["briefing_title"].format(date=today), expanded=True):
        ...
        wa_lines = [
            "\U0001F4CA *Briefing Equity Guard*",
            f"*{today}*",
            "",
            "\U0001F3E6 *Juros*",
            f"\U0001F1FA\U0001F1F8 Fed: {FED_FUNDS_RATE:.2f}% (FOMC {_fmt_date_br(FED_NEXT_MEETING)})",
            f"\U0001F1E7\U0001F1F7 Selic: {SELIC_RATE:.2f}% (COPOM {_fmt_date_br(SELIC_NEXT_MEETING)})",
            "",
            "\U0001F6E2 *Commodities*",
            f"Brent US$ {_fmt_val('Brent')} {_wa_chg('Brent')}",
            f"WTI US$ {_fmt_val('WTI')} {_wa_chg('WTI')}",
            "",
            "\U0001F4B5 *Dolar Comercial*",
            ...
            "",
            "\U0001F3E6 *Previdencia ({db})*".format(db=_pd['data_base']),
            ...
            "\U0001F449 equityguard.streamlit.app",
        ]
